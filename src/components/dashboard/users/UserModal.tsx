"use client";
import React from "react";
import { Modal } from "@/components/ui/modal";
import Input from "@/components/form/input/InputField";
import Label from "@/components/form/Label";
import Select from "@/components/form/Select";

interface UserModalProps {
    isOpen: boolean;
    onClose: () => void;
    roleType?: string; // 'ADMIN' | 'PRINCIPAL' | 'TEACHER' | etc.
}

const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose, roleType = "ADMIN" }) => {
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Handle user creation logic here
        onClose();
    };

    const roleLabel = roleType.charAt(0).toUpperCase() + roleType.slice(1).toLowerCase();

    return (
        <Modal isOpen={isOpen} onClose={onClose} className="max-w-xl">
            <div className="p-8">
                <div className="mb-8">
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Add New {roleLabel}</h2>
                    <p className="text-gray-500 dark:text-gray-400 mt-1">Provide the user details below to create a new {roleLabel.toLowerCase()} account.</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <Label htmlFor="userName">Full Name</Label>
                            <Input
                                id="userName"
                                placeholder="e.g. John Doe"
                                name="name"
                                type="text"
                                required
                            />
                        </div>
                        <div>
                            <Label htmlFor="userEmail">Email Address</Label>
                            <Input
                                id="userEmail"
                                placeholder="e.g. john@example.com"
                                name="email"
                                type="email"
                                required
                            />
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <Label htmlFor="userPhone">Phone Number</Label>
                            <Input
                                id="userPhone"
                                placeholder="+1 234 567 890"
                                name="phone"
                                type="tel"
                            />
                        </div>
                        {roleType === 'ADMIN' && (
                            <div>
                                <Label htmlFor="userRole">Specific Role</Label>
                                <Select
                                    id="userRole"
                                    options={[
                                        { label: 'Admin', value: 'ADMIN' },
                                        { label: 'Super Admin', value: 'SUPER_ADMIN' },
                                    ]}
                                    defaultValue="ADMIN"
                                />
                            </div>
                        )}
                    </div>

                    <div>
                        <Label htmlFor="userAddress">Address</Label>
                        <Input
                            id="userAddress"
                            placeholder="e.g. 123 Street Name, City"
                            name="address"
                            type="text"
                        />
                    </div>

                    <div className="flex justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-800">
                        <button
                            type="button"
                            onClick={onClose}
                            className="px-6 py-2.5 text-sm font-semibold text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-all"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            className="px-6 py-2.5 text-sm font-semibold bg-blue-600 text-white hover:bg-blue-700 rounded-xl transition-all shadow-lg shadow-blue-200 dark:shadow-none"
                        >
                            Create {roleLabel}
                        </button>
                    </div>
                </form>
            </div>
        </Modal>
    );
};

export default UserModal;
